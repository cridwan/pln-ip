...
const props = defineProps({
    ...
    document: {
        type: Object as PropType<ResponseDocumentInterface>
    }
});
    ...
    <DialogRoot :open="show" @update:open="handleOpenChange">
        ...
        <DialogPortal>
            ...
            <DialogContent class="v-modal-content" :style="styles">
                ...
                <div class="v-modal-body overflow-hidden" :style="styles">
                    <video autoplay controls :src="parsedUrl(document?.document_link)"
                        class="w-full h-full object-cover" v-if="document"></video>
                    <span v-else>Terjadi kesalahan saat render video</span>
                </div>
                <DialogClose v-if="showButtonClose" class="v-modal-close" aria-label="Close">
