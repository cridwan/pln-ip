<template>
    <div :class="[
        ...
        <div class="card-content">
            <h3 class="text-xl font-bold text-blue-950 mb-4 border-b pb-3 border-blue-50">
                {{ project.name }}
            </h3>
            ...
            <div class="space-y-3">
                <div class="flex justify-between items-center text-sm">
                    ...
                    <span
                        :class="['font-semibold', statusTextColorClass, 'px-2', 'py-0.5', 'rounded-full', 'bg-opacity-10', statusBgClass]">
                        {{ project.status }}
                    </span>
                </div>
                ...
                <div class="flex justify-between items-center text-sm border-t pt-3 border-gray-100">
                    ...
                    <span class="font-semibold text-gray-700">{{ project.createdBy }}</span>
                </div>
...
const props = defineProps({
    project: {
        ...
        required: true,
        default: () => ({
            name: 'Nama Proyek Contoh',
            status: 'In Progress', // 'Completed', 'In Progress', 'Pending', 'Cancelled'
            createdBy: 'Pengguna Tailwind',
            dateCreated: '2025-11-20' // Format ISO
        })
    }
});
...
const formattedDate = computed(() => {
    try {
        return new Date(props.project.dateCreated).toLocaleDateString('id-ID', { year: 'numeric', month: 'long', day: 'numeric' });
    } catch (e) {
