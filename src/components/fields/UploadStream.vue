  ...
  <div>
    <div v-if="props.selectedValues" class="relative">
      ...
      <video
        ...
        "
      ></video>
      <a target="_blank" :href="parsedUrl()" v-else class="text-blue-950">{{
        props.selectedValues?.document_link
      }}</a>
    </div>
    <div v-else>
      ...
      <div
        :class="[
          'w-full border  rounded-md p-4 flex items-center justify-center flex-col gap-3',
          className,
        ]"
      >
        <input
        ...
        />
        <button
          class="bg-blue-900 text-white py-1 px-2 rounded-sm"
          type="button"
          @click="handleClick"
        >
          Choose File
        ...
        <span class="text-sm text-gray-600 italic" v-if="!file">
          * Format file yang diperbolehkan (format
          {{ props.fileType.join(", ") }})
        </span>
      ...
      <span class="text-red-500 text-xs" v-if="checkErrorUpload()"
