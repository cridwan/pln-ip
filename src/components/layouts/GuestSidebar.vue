...
import { MenusGuest } from "@/constants/Menus";
                ...
                <div v-else class="flex flex-col gap-2" @click="openChildren(item)">
                    ...
                    <div v-if="isActive(item)" class="pl-5 flex flex-col gap-2">
                        <RouterLink v-for="(element, index) in item.children" :key="index" :to="item.url === '/'
                            ? `/${route.params?.id}/guest/${route.params?.id_unit}/${route.params?.id_machine}?sequence=${route?.params?.id_inspection}`
                            : `/${route.params?.id}/guest/${route.params?.id_unit}/${route.params?.id_machine}/${route?.params?.menu}/${route?.params?.id_project}/${route?.params?.id_inspection}${element.url}`
                            " :class="item.url === '/'
                                ? ''
