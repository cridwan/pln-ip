                ...
                <div v-else class="flex flex-col gap-2" @click="openChildren(item)">
                    ...
                    <div v-if="isActive(item)" class="pl-5 flex flex-col gap-2">
                        <RouterLink v-for="(element, index) in item.children" :key="index"
                            :to="`/${route.params?.id}/guest/${route.params?.id_unit}/${route.params?.id_machine}/${route?.params?.menu}/${route?.params?.id_project}/${route?.params?.id_inspection}/add-scope/${route?.params?.id_scope}${element.url}?original_uuid=${route.query.original_uuid}`"
                            :class="item.url === '/'
                                ? ''
