...
const { isPending: isLoadingRequest, mutate: approveRequestProject } = useMutation({
    ...
    <div class="w-full flex items-center justify-center bg-white p-4 shadow-md rounded-sm">
        <div class="flex flex-col items-center gap-3">
            ...
            <p class="text-blue-950 tracking-wide font-normal text-lg text-center"
                ...
                Silahkan pilih user untuk melakukan request approve project {{ dataProject?.name }}
            </p>
            ...
            <div class="w-full" v-show="dataProject?.status == 'pending'">
                <Select v-model="model.user_id" label="Pilih user approval" options_label="label" options_value="value"
            ...
            <Button :text="dataProject?.status == 'pending' ? 'Request Approval' : 'Disable'"
                v-show="dataProject?.status == 'pending'" :color="dataProject?.status == 'pending' ? 'blue' : 'green'"
                :loading="isLoadingRequest" variant="primary" size="lg" icon_before="check-list" type="button"
