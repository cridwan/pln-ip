...
const model = ref<FilterToolStdInterface>({
  bidang_uuid: "",
  sub_bidang_uuid: "",
  scope_standart_uuid: "",
  equipment_uuid: "",
...
const {
  ...
  queryKey: ["getScopeFilterToolStd"],
  enabled: false,
  queryFn: async ({ pageParam = 1 }) => {
    try {
...
const {
  ...
  queryKey: ["getEquipmentFilterToolStd"],
  enabled: false,
  queryFn: async ({ pageParam = 1 }) => {
    try {
...
const {
  ...
  queryKey: ["getActivityFilterToolStd"],
  enabled: false,
  queryFn: async ({ pageParam = 1 }) => {
    try {
...
const handleSubmit = async () => {
  ...
  emit(
    "filter",
    model.value,
    dataActivity.value?.pages
      ?.flatMap((page) => page?.data)
      ?.find((item) => item.uuid == model.value.activity_uuid)
  );
};
