...
<template>
  <Modal width="440" height="200" :showButtonClose="false" :title="props.selectedValue
    ...
    : 'Tambah Consumbale Material'
    " v-model="modelValue">
    <form class="flex flex-col gap-4 max-h-[calc(100vh-200px)] overflow-y-auto mx-[-20px] px-5"
      @submit.prevent="handleSubmit">
      <Input v-model="model.name" star label="Nama Consumable Material" :rules="rules.name"
        :custom_symbols="all_characters" :disabled="Number(selectedValue?.has_transaction || 0) > 0" />
      <Input v-model="model.price" star label="Harga" :rules="rules.price" :custom_symbols="numbers_positive"
        :is_currency="true" />
      <Input v-model="model.merk" :disabled="Number(selectedValue?.has_transaction || 0) > 0" star label="Merk"
        :rules="rules.merk" :custom_symbols="all_characters" />
      <Select v-model="model.global_unit_uuid" :disabled="Number(selectedValue?.has_transaction || 0) > 0" star
