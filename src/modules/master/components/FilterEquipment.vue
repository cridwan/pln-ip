...
const {
  ...
  queryKey: ["getLocationFilterEquipmentMaster"],
...
const {
  ...
  queryKey: ["getUnitFilterEquipmentMaster"],
  enabled: false,
  queryFn: async ({ pageParam = 1 }) => {
    try {
...
const {
  ...
  queryKey: ["getMachineFilterEquipmentMaster"],
  enabled: false,
  queryFn: async ({ pageParam = 1 }) => {
    try {
...
const {
  ...
  queryKey: ["getInspectionFilterEquipmentMaster"],
  enabled: false,
  queryFn: async ({ pageParam = 1 }) => {
    try {
...
const selectLocation = (e: OptionType) => {
  queryClient.removeQueries({ queryKey: ["getMachineFilterEquipmentMaster"] });
  queryClient.removeQueries({
    queryKey: ["getInspectionFilterEquipmentMaster"],
  });
  queryClient.removeQueries({ queryKey: ["getBidangFilterEquipmentMaster"] });
  queryClient.removeQueries({
    queryKey: ["getSubBidangFilterEquipmentMaster"],
  });
  queryClient.removeQueries({ queryKey: ["getScopeFilterEquipmentMaster"] });
    ...
    if (props.selectedValue) {
      ...
      options_location.value = mergeArrays(
        [
          {
            value:
              props.selectedValue?.scope_standart?.inspection_type?.machine
                ?.unit?.location_uuid,
            label:
              props.selectedValue?.scope_standart?.inspection_type?.machine
                ?.unit?.location?.name,
    ...
    if (props.selectedValue) {
      ...
      options_machine.value = mergeArrays(
        [
          {
            value:
              props.selectedValue?.scope_standart?.inspection_type
                ?.machine_uuid,
            label:
              props.selectedValue?.scope_standart?.inspection_type?.machine
                ?.name,
  ...
  (newInspection) => {
    if (props.selectedValue) {
      ...
      options_inspection.value = mergeArrays(
        [
          {
            value: props.selectedValue?.scope_standart?.inspection_type_uuid,
            label: props.selectedValue?.scope_standart?.inspection_type?.name,
          },
        ],
        new_data.filter(
          (item) =>
            item.value !==
            props.selectedValue?.scope_standart?.inspection_type?.uuid
        )
      );
    } else {
      const new_data: OptionType[] =
...
watch(
  ...
  (value) => {
    if (value) {
      if (value?.scope_standart?.inspection_type?.machine?.unit?.location) {
        refetchLocation();
      }

      if (value?.scope_standart?.inspection_type?.machine?.unit) {
        params_unit.filters = [
          {
            group: "AND",
            operator: "EQ",
            column: "location_uuid",
            value:
              value?.scope_standart?.inspection_type?.machine?.unit
                ?.location_uuid,
          },
        ];
        refetchUnit();
      ...
      if (value.scope_standart?.inspection_type?.machine) {
        params_machine.filters = [
          {
            group: "AND",
            operator: "EQ",
            column: "unit_uuid",
            value: value?.scope_standart?.inspection_type?.machine?.unit_uuid,
          },
        ];
        refetchMachine();
      }

      if (value?.scope_standart?.inspection_type) {
        params_inspection.filters = [
          {
            group: "AND",
            operator: "EQ",
            column: "machine_uuid",
            value: value?.scope_standart?.inspection_type?.machine_uuid,
          },
        ];
        refetchInspection();
      }
