...
import { ColumnsToolStd } from "../constants/ToolStdConstant";
...
const { mutate: deleteToolStd, isPending: isLoadingDelete } = useMutation({
  mutationFn: async (id: string) => {
    return await masterStore.deleteToolStd(id);
  },
  onSuccess: () => {
...
const { mutate: downloadToolStd, isPending: isLoadingDownload } = useMutation({
  mutationFn: async () => {
    return await masterStore.downloadToolStd(params);
  },
  onSuccess: () => {},
  onError: (error) => {
    console.log(error);
...
const { mutate: templateToolStd, isPending: isLoadingTemplate } = useMutation({
  mutationFn: async () => {
    return await masterStore.templateToolStd();
  },
  onSuccess: () => {},
  onError: (error) => {
    console.log(error);
...
const { mutate: importToolStd, isPending: isLoadingImport } = useMutation({
  mutationFn: async (payload: File) => {
    return await masterStore.importToolStd(payload);
  },
  onSuccess: () => {
    toastRef.value?.showToast({
      title: "Success",
